        ...
        while lock == False:
            ...
            try:
                ...
                for a in range(0, 250):
                    character = port.read()
                    line += character
                    if (line[0:2] == b"\xa5\x5a"):
                        if(len(line) == 7):
                            lock = True
                            break

                    elif (line[0:2] != b"\xa5\x5a" and len(line) == 2):
                        line = b""
...
################################
    ...
    def point_Polar(self, serial_frame, radians=False):
        # Get Distance
        distance = binascii.b2a_hex(struct.pack("B",
                                                serial_frame[4])) + binascii.b2a_hex(struct.pack("B", serial_frame[3]))
        distance = int(distance, 16)
        distance = distance / 4  # instructions from data sheet
        # Get Angle
        angle = binascii.b2a_hex(
            struct.pack("B", serial_frame[2])) + binascii.b2a_hex(struct.pack("B", serial_frame[1]))
        # remove check bit, convert to integer
        angle = self.leftshiftbits(angle)
